...
class BlenderMCPServer:
    ...
    def _execute_command_internal(self, command):
        ...
        handlers = {
            ...
            "get_artist_profile": self.get_artist_profile,
        }
    ...
    def get_artist_profile(self):
        """Get the currently selected artist profile from the BlenderMCP panel"""
        try:
            profile_key = bpy.context.scene.blendermcp_artist_profile
            profiles = {
                'NONE': ('None', 'No profile selected'),
                'GAME_ASSET': ('Game Asset Artist', 'Game-ready, optimized, engine-compatible output. FBX export, PBR materials, poly budgets, collision geometry.'),
                'FILM_VFX': ('Film/VFX Artist', 'Photorealism, EXR multi-pass, Cycles, ACES workflow. Versioning before render, test renders.'),
                'ARCHVIZ': ('Architectural Visualization', 'Real-world scale, 3 lighting variants (day/golden hour/overcast), EEVEE previews for clients.'),
                'MOTION_GRAPHICS': ('Motion Graphics / Advertising', 'Logo reveals, product shots, broadcast-safe rules. Studio lighting, turntable animations.'),
                'CHARACTER': ('Character Artist', 'Full pipeline: base mesh -> sculpt -> retopo -> UV -> bake -> rig. Skin weights, humanoid rig.'),
                'ENVIRONMENT': ('Environment Artist', 'ANT terrain, HDRI lighting, GeoNodes scatter systems. Modular kit assembly, blockout first.'),
                'MEDICAL': ('Medical / Scientific Artist', 'DICOM import, SSS materials, 3D print validation. Anatomical naming conventions.'),
                'INDUSTRIAL': ('Industrial / Product Design', 'Precision modeling (mm units), 3D printing validation, studio lighting. CAD import handling.'),
                'AR_VR': ('AR/VR/Metaverse Artist', 'LOD generation, lightmap baking, GLB export, strict performance budgets. 90fps target for VR.'),
                'EDUCATIONAL': ('Educational / Simulation Artist', 'Custom properties, collision meshes, WebGL export. Accessibility checks, step-by-step training.'),
            }
            info = profiles.get(profile_key, ('Unknown', 'No description'))
            return {
                "active": profile_key != 'NONE',
                "key": profile_key,
                "name": info[0],
                "description": info[1],
            }
...
class BLENDERMCP_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.separator()
        layout.label(text="Artist Profile:", icon='WORKSPACE')
        layout.prop(scene, "blendermcp_artist_profile", text="")
        if scene.blendermcp_artist_profile != 'NONE':
            profile_box = layout.box()
            profile_box.label(text={
                'GAME_ASSET': 'Game Asset Artist',
                'FILM_VFX': 'Film/VFX Artist',
                'ARCHVIZ': 'ArchViz Artist',
                'MOTION_GRAPHICS': 'Motion Graphics / Advertising',
                'CHARACTER': 'Character Artist',
                'ENVIRONMENT': 'Environment Artist',
                'MEDICAL': 'Medical / Scientific Artist',
                'INDUSTRIAL': 'Industrial / Product Design',
                'AR_VR': 'AR/VR/Metaverse Artist',
                'EDUCATIONAL': 'Educational / Simulation Artist',
            }.get(scene.blendermcp_artist_profile, 'Unknown'), icon='DOT')
            profile_box.label(text={
                'GAME_ASSET': 'Game-ready, optimized, engine-compatible output',
                'FILM_VFX': 'Photorealism, EXR multi-pass, Cycles, ACES workflow',
                'ARCHVIZ': 'Real-world scale, 3 lighting variants, EEVEE previews',
                'MOTION_GRAPHICS': 'Logo reveals, product shots, broadcast-safe',
                'CHARACTER': 'Base mesh -> sculpt -> retopo -> UV -> bake -> rig',
                'ENVIRONMENT': 'ANT terrain, HDRI lighting, GeoNodes scatter',
                'MEDICAL': 'DICOM import, SSS materials, 3D print validation',
                'INDUSTRIAL': 'Precision modeling, 3D printing, studio lighting',
                'AR_VR': 'LOD generation, GLB export, performance budgets',
                'EDUCATIONAL': 'Custom properties, collision, WebGL export',
            }.get(scene.blendermcp_artist_profile, ''))
...
def register():
    ...
    bpy.types.Scene.blendermcp_artist_profile = bpy.props.EnumProperty(
        name="Artist Profile",
        description="Select an artist profile to guide MCP behavior, materials, export settings, and naming conventions",
        items=[
            ("NONE", "None (Default)", "No artist profile selected"),
            ("GAME_ASSET", "Game Asset Artist", "Game-ready, optimized, engine-compatible output"),
            ("FILM_VFX", "Film/VFX Artist", "Photorealism, EXR multi-pass, Cycles, ACES workflow"),
            ("ARCHVIZ", "Architectural Visualization", "Real-world scale, 3 lighting variants, EEVEE previews"),
            ("MOTION_GRAPHICS", "Motion Graphics / Advertising", "Logo reveals, product shots, broadcast-safe"),
            ("CHARACTER", "Character Artist", "Base mesh -> sculpt -> retopo -> UV -> bake -> rig"),
            ("ENVIRONMENT", "Environment Artist", "ANT terrain, HDRI lighting, GeoNodes scatter"),
            ("MEDICAL", "Medical / Scientific Artist", "DICOM import, SSS materials, 3D print validation"),
            ("INDUSTRIAL", "Industrial / Product Design", "Precision modeling, 3D printing, studio lighting"),
            ("AR_VR", "AR/VR/Metaverse Artist", "LOD generation, GLB export, performance budgets"),
            ("EDUCATIONAL", "Educational / Simulation Artist", "Custom properties, collision, WebGL export"),
        ],
        default="NONE"
    )
...
def unregister():
    ...
    del bpy.types.Scene.blendermcp_artist_profile
